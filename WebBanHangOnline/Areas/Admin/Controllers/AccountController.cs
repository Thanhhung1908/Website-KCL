        ...
        public async Task<ActionResult> Edit(string id)
      {
            ...
            foreach (var item1 in items)
            {
                if(item1.Id == id)
                {
                    ...
                    var roleCollect = roleTemp[0];
                    var roleId = db.Roles.SingleOrDefault(x => x.Name.Equals(roleCollect));
                    var viewModel = new CreateAccountViewModel()
                    {
                        UserName = item1.UserName,
                        Password = item1.PasswordHash,
                        FullName = item1.FullName,
                        Phone = item1.Phone,
                        Email = item1.Email,
                        Role = roleId.Id,
                        ConfirmPassword = item1.PasswordHash
                    };
                    //var item = item1;
